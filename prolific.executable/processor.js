...
class Processor extends events.EventEmitter {
    constructor (module) {
        ...
        this._configured = new Future
        this.configured = this._configured.promise
        ...
        this._reconfigurator = new Reconfigurator(this._path, new class extends BufferConfigurator {
            configure (buffer) {
                ...
                const process = processor.process(require('prolific.require'))
                return { source, triage, buffer }
            }
        })
        this._reconfigurator.on('error', this.emit.bind(this, 'error'))

        this._destructible.durable('reconfigure', this._reconfigurator.monitor())
    ...
    async configure () {
        const { source, triage, process } = this._reconfigurator.shift()
